...
def as_list_of_lists(x): return [as_list(i) for i in x]
...
def parse_config(config: dict) -> dict:
    ...
    conf['dates_range'] = [pd.to_datetime(config['dates_range'][0]),
                           pd.to_datetime(
                               config['dates_range'][1])+pd.Timedelta(days=1)]
    for src in conf['min_hour_depart'].keys():
        conf['min_hour_depart'][src] = pd.to_datetime(
            conf['min_hour_depart'][src], format='%H:%M').time()
